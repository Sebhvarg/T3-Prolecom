...
class TemaController extends Controller
{
    ...
    public function destroy(Request $request, $id)
    {
        ...
        foreach ($tema->items as $item) {
            if ($item->itemable) {
                if ($item->itemable_type === MaterialAprendizaje::class && Storage::disk('local')->exists($item->itemable->enlaceArchivo)) {
                    Storage::disk('local')->delete($item->itemable->enlaceArchivo);
                }
